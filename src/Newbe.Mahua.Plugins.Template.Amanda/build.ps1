...
Task PackCleverQQ -depends DonwloadPackages, Build -Description "CleverQQ打包" {
    $InstalledPlatforms | Where-Object {$_.id -eq "Newbe.Mahua.CleverQQ"}| ForEach-Object {
        Exec {
            ...
            Copy-Item "$releaseBase\CleverQQ\$pluginName" "$releaseBase\CleverQQ\$assetDirName\$pluginName.IR" -Recurse
            Get-ChildItem "$releaseBase\CleverQQ\$assetDirName\$pluginName.IR" | Get-FileHash | Out-File "$releaseBase\hash.txt"
            Copy-Item "$releaseBase\hash.txt" "$releaseBase\CleverQQ\$assetDirName\$pluginName.IR\hash.txt"
            Remove-Item "$releaseBase\hash.txt"
